...
namespace arrayTweaks {
	...
	void printArrayLog(const vector<complex<T>>& arr, string message = "") {
		...
		cout << fixed;
		for (size_t i = 0; i < arr.size(); i += 1) {
			cout << getSpacedElement(arr[i].real()) << " "
				<< getSpacedElement(arr[i].imag()) << endl;
		}
...
vector<complex<double>> getSemiFastFourierTransform(const vector<double>& data) {
	...
	size_t p1 = sqrt(data.size());
	size_t p2 = p1;
	const double REVERSE_P1 = 1.0 / static_cast<double>(p1);
	const double REVERSE_P2 = 1.0 / static_cast<double>(p2);
	...
	vector<complex<double>> resultArray(p1 * p2);

	for (size_t k1 = 0; k1 < p1; k1 += 1) {
		for (size_t j2 = 0; j2 < p2; j2 += 1) {
			for (size_t j1 = 0; j1 < p1; j1 += 1) {
	...
	for (size_t k1 = 0, j = 0; k1 < p1; k1 += 1) {
		for (size_t k2 = 0; k2 < p2; k2 += 1, j += 1) {
			resultArray[j] = temp2[k1][k2];
		}
	}
